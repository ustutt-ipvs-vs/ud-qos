...
// Parameters
#define INTERVAL_IN_S 60
// for monitoring
#define NUMBER_IPv4 3
#define NUMBER_IPv6 3
const int UPDATE_PERIODE_IN_MS = 1000;
/////////////

struct monitoring_ipv6 {
	struct in6_addr ip;
	unsigned int sumPacketSizes[INTERVAL_IN_S];
};
...
unsigned int current_index_monitoring = INTERVAL_IN_S - 1;
unsigned int last_index_monitoring = 0;
...
static unsigned int get_dataRate_ipv6(struct in6_addr ip)
{
	for(int i = 0; i < NUMBER_IPv6; i++)
	{
		if(ipv6_addr_equal(&ip, &mon_ipv6[i].ip))
		{
			return mon_ipv6[i].sumPacketSizes[current_index_monitoring] / INTERVAL_IN_S;   
		}
	}
...
static unsigned int get_dataRate_ipv4(unsigned int ip)
{
	for(int i = 0; i < NUMBER_IPv4; i++)
	{
		if(mon_ipv4[i].ip == ip)
		{
			return mon_ipv4[i].sumPacketSizes[current_index_monitoring] / INTERVAL_IN_S;
		}
	}
...
static int get_dataRate_class(struct sk_buff *skb, struct Qdisc *sch)
{
	...
	if (skb->protocol == htons(ETH_P_IPV6)) {
		...
		dataRate = get_dataRate_ipv6(src_ip);
		...
		dataRate = get_dataRate_ipv4(src_ip);
...
static enum hrtimer_restart update_dataRate(struct hrtimer *timer)
{
	...
	// Update SlidingWindow index
	if (current_index_monitoring == 0)
	{
		last_index_monitoring = current_index_monitoring;
		current_index_monitoring = INTERVAL_IN_S - 1;
	} else {
		last_index_monitoring = current_index_monitoring;
		current_index_monitoring = current_index_monitoring - 1;
	}
	
	// update SlidingWindow dataStructure
	for(int i = 0; i < NUMBER_IPv4; i++)
	{
		spin_lock(&mon_lock_ipv4);
		mon_ipv4[i].sumPacketSizes[last_index_monitoring] = 0;
		spin_unlock(&mon_lock_ipv4);		
	}
	for(int i = 0; i < NUMBER_IPv6; i++)
	{
		spin_lock(&mon_lock_ipv6);
		mon_ipv6[i].sumPacketSizes[last_index_monitoring] = 0;
...
void monitoring_pkt_ipv6(struct in6_addr ip, unsigned int size)
{
	for (int i = 0; i < NUMBER_IPv6; i++)
	{
		if (ipv6_addr_equal(&ip, &mon_ipv6[i].ip))
		{
			for (int k = 0; k < INTERVAL_IN_S; k++)
			{
				spin_lock(&mon_lock_ipv6);
				mon_ipv6[i].sumPacketSizes[k] = mon_ipv6[i].sumPacketSizes[k] + size;
				spin_unlock(&mon_lock_ipv6);
			}
			return;
		}
...
void monitoring_pkt_ipv4(unsigned int ip, unsigned int size)
{
	for (int i = 0; i < NUMBER_IPv4; i++)
	{
		if (ip == mon_ipv4[i].ip)
		{
			...
			{
				spin_lock(&mon_lock_ipv4);
				mon_ipv4[i].sumPacketSizes[k] = mon_ipv4[i].sumPacketSizes[k] + size;
				spin_unlock(&mon_lock_ipv4);
			}
			return;
		}
...
{
    printk(KERN_INFO "Load qdisc ud-qos\n");
	return register_qdisc(&ud_qos_qdisc_ops);
}

static void __exit prio_module_exit(void)
{
    printk(KERN_INFO "Exit qdisc ud-qos\n");
