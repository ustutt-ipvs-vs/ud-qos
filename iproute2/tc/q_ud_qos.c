...
{
	fprintf(stderr, "Usage: ... ud_qos bands NUMBER_CLASSES\n");
}

static int ud_qos_parse_opt(struct qdisc_util *qu, int argc, char **argv,
...
int ud_qos_print_opt(struct qdisc_util *qu, FILE *f, struct rtattr *opt)
{
	int i;
...
struct qdisc_util ud_qos_qdisc_util = {
	.id		= "ud_qos",
	.parse_qopt	= ud_qos_parse_opt,
	.print_qopt	= ud_qos_print_opt,
};
